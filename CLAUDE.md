...
- `seedIfEmpty()` loads [`src/seed/seedDishes.js`](src/seed/seedDishes.js) **only when the store is empty**, so deleting a dish in-app keeps it gone and re-running never duplicates. `App` calls it once on mount, then reads all dishes into state.
...
- To re-trigger seeding during development, clear the `mixiao-cooks` IndexedDB database in browser devtools.

**Dish data model** (see the brief for the canonical list): `id, name, origin[], timeCategory, ingredients[], appliances[], steps[], photos[], timers[]`. Notes that affect logic:
...
`AddDish` covers every data-model field (name, multi-select/add-your-own origin tags, time category, repeated ingredients/appliances/steps, photo upload, optional named timers). Photos are stored as `File`/`Blob` objects directly (IndexedDB structured-clones them). The same form handles **editing**: when passed a `dish` prop it pre-fills and saves via `updateDish` (preserving the id); otherwise it calls `addDish` (which generates the `id` via `crypto.randomUUID()`). Editing is reached from the dish detail's "edit dish" control; "delete" calls `deleteDish` after a confirm. After any write, `App` re-fetches `getAllDishes()` into state so changes show immediately — the dishes list is loaded once and must be refreshed this way.
